...
const BF2TOP_XML_PATH = path.resolve(__dirname, 'server', 'bf2top.xml');
...
const httpHandler = async (req, res, body) => {
  ...
  if (method === 'POST' && url === '/restart') {
    console.log(`/restart endpoint hit.`);
    try {
      // Handle users.xml
      if (body.usersxml) {
        console.log('Writing to file:', USERS_XML_PATH, 'with data:', body.usersxml);
        console.log('Saving users.xml from request body...');
        ...
        console.log(`users.xml saved to ${USERS_XML_PATH}`);
      } else {
        console.warn('No users.xml provided in the request body');
      }

      // Handle bf2top.xml
      if (body.profilexml) {
        console.log('Writing to file:', BF2TOP_XML_PATH, 'with data:', body.profilexml);
        console.log('Saving bf2top.xml from request body...');
        ...
        console.log(`bf2top.xml saved to ${BF2TOP_XML_PATH}`);
      } else {
        console.warn('No bf2top.xml (profilexml) provided in the request body');
      }

      // Determine the restart mode and prepare arguments
      const mode = body.mode;
      if (mode === 'infantry' || mode === 'vehicles') {
        const args = [body.mapName || ''];

		if (hasProfileXml) {
			// If profilexml is present, second arg is always 'bf2top'
			args.push('bf2top');
        ...
        console.log(`Restarting server with mode: ${mode}`);
      ...
      console.error(`Error in /restart: ${error.message}`);
      res.statusCode = 500;
